...
CREATE INDEX IF NOT EXISTS idx_agent_memory_ns ON t_agent_memory(namespace);
CREATE INDEX IF NOT EXISTS idx_agent_memory_ns_prefix ON t_agent_memory(namespace text_pattern_ops);
...
COMMENT ON TABLE t_agent_memory IS 'Agent 长期记忆表';
COMMENT ON COLUMN t_agent_memory.namespace IS '命名空间，点分格式';
COMMENT ON COLUMN t_agent_memory.key IS '记忆键';
COMMENT ON COLUMN t_agent_memory.user_id IS '用户ID';
COMMENT ON COLUMN t_agent_memory.memory_kind IS '记忆类型：profile/episodic';
COMMENT ON COLUMN t_agent_memory.scope_type IS '作用域类型';
COMMENT ON COLUMN t_agent_memory.scope_id IS '作用域ID';
COMMENT ON COLUMN t_agent_memory.slot IS '稳定槽位';
COMMENT ON COLUMN t_agent_memory.source_record_id IS '来源问答记录ID';
COMMENT ON COLUMN t_agent_memory.confidence IS '提取置信度';
COMMENT ON COLUMN t_agent_memory.value IS '记忆值（JSONB）';
COMMENT ON COLUMN t_agent_memory.embedding IS '向量数据（pgvector）';
COMMENT ON COLUMN t_agent_memory.created_at IS '创建时间';
COMMENT ON COLUMN t_agent_memory.updated_at IS '更新时间';
COMMENT ON COLUMN t_agent_memory.expires_at IS '过期时间';
